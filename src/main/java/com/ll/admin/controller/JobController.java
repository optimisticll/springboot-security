...
public class JobController {
    ...
    public String addJob(Model model){
        model.addAttribute("MyJob",new Job());
    ...
    @PreAuthorize("hasAnyAuthority('job:add')")
    @Log("添加岗位")
    public Result saveJob(@RequestBody Job job){
        if (UserConstants.JOB_NAME_NOT_UNIQUE.equals(jobService.checkJobNameUnique(job))) {
            return Result.error().message("新增岗位'" + job.getJobName() + "'失败，岗位名称已存在");
        }
        return Result.judge(jobService.insertJob(job),"添加岗位");
    }

    @GetMapping(value = "/edit")
    @ApiOperation(value = "修改岗位页面")
    @PreAuthorize("hasAnyAuthority('job:edit')")
    public String editRole(Model model, Job job) {
    ...
    public Result updateJob(@RequestBody Job job){
        if (UserConstants.JOB_NAME_NOT_UNIQUE.equals(jobService.checkJobNameUnique(job))) {
            return Result.error().message("修改岗位'" + job.getJobName() + "'失败，岗位名称已存在");
        }
        return Result.judge(jobService.updateJob(job),"修改岗位");
    }
    /**
     * 用户状态修改
     */
    ...
    @PreAuthorize("hasAnyAuthority('job:edit')")
    public Result changeStatus(@RequestBody Job job)
    {
    ...
    public Result<Job> deleteRole(String ids) {
        try {
