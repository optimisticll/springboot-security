...
public class DictDetailController {
    ...
    public String addDictDetail(Model model, DictDetail dictDetail){
        model.addAttribute("MyDictDetail",dictDetail);
    ...
    @PreAuthorize("hasAnyAuthority('dict:add')")
    @Log("添加字典详情")
    public Result saveDictDetail(@RequestBody DictDetail dictDetail){
        return Result.judge(detailService.insertDictDetail(dictDetail),"添加字典详情");
    }

    @GetMapping(value = "/edit")
    @ApiOperation(value = "修改字典详情页面")
    @PreAuthorize("hasAnyAuthority('dict:edit')")
    public String editDictDetail(Model model, DictDetail dictDetail) {
        model.addAttribute("MyDictDetail",detailService.getDictDetailById(dictDetail.getId()));
    ...
    @PreAuthorize("hasAnyAuthority('dict:edit')")
    @Log("修改字典详情")
    public Result updateDictDetail(@RequestBody DictDetail dictDetail){
        return Result.judge(detailService.updateDictDetail(dictDetail),"修改字典详情");
    }
